    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                onkeyup="ChangeToSlug();" id="slug" value="{{ old('name', $category->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                id="convert_slug" value="{{ old('slug', $category->slug) }}">
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                    @foreach($parents as $id => $parent)
                        <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $category->parent->id ?? '') == $id ? 'selected' : '' }}>
                            {{ $parent }}
                        </option>
                    @endforeach
                </select>
