...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/custom-btn.css') }}">
    @yield('styles')
</head>
    ...
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
            ...
            <ul class="navbar-nav ml-auto">
              <span>Hi:</span>
                    @if(Auth::check())
                        <Strong style="color: red">{{Auth::user()->name}}</Strong>
                    @endif
...
<script type="text/javascript">
  ...
  var query= 
  <?php 
  echo json_encode((object)Request::only(['category', 'author', 'status', 'start', 'end']));
  ?>;
  function filter(){
      Object.assign(query,{'category': $('#category_filter').val()});
      Object.assign(query,{'author': $('#author_filter').val()});
      Object.assign(query,{'status': $('#status_filter').val()});
      Object.assign(query,{'start': $('#start_filter').val()});
      Object.assign(query,{'end': $('#end_filter').val()});
    window.location.href="{{route('admin.posts.index')}}?"+$.param(query);
  }
  </script>
